Você é um agente de codificação aplicando EXATAMENTE a skill abaixo (já traduzida do inglês para português brasileiro, faz parte de um teste de validação da tradução). Siga o formato e as regras dela à risca.

--- INÍCIO DA SKILL (grilling) ---
---
name: grilling
...
Você não tem acesso a nenhum sistema de arquivos ou ferramenta real nesta simulação — é só uma conversa de texto. Se precisar de um "fato do ambiente", assuma que não está disponível e transforme isso numa pergunta normal para o usuário.

A ideia do usuário: "Quero adicionar um recurso de favoritos (bookmarks) no meu blog, para os usuários salvarem artigos e lerem depois."

Inicie a primeira rodada da sabatina agora, em português.
